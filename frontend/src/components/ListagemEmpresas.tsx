// =============================================================================
// src/components/ListagemEmpresas.tsx — Tabela de Empresas com CRUD completo
//
// Funcionalidades:
//   - Listagem de empresas ativas com carregamento (spinner)
...
import { empresaService, type EmpresaListada, type CadastrarEmpresaPayload } from '../services/api';
...
function ModalEdicao({ empresa, onFechar, onSalvo }: ModalEdicaoProps) {
  const [form, setForm] = useState<FormEdicao>({
    razao_social:  empresa.razao_social,
    nome_fantasia: empresa.nome_fantasia ?? '',
    contato_nome:  empresa.contato_nome ?? '',
    email_login:   empresa.usuario.email_login,
    senha:         '',
  });
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setSalvando(true);
    try {
      // Monta payload apenas com campos relevantes
      const payload: Partial<CadastrarEmpresaPayload> = {
      ...
      await empresaService.atualizar(empresa.id_empresa, payload);
      ...
      >
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-lg font-bold text-slate-900">✏️ Editar Empresa</h2>
            <button
              onClick={onFechar}
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-3 py-2 rounded-lg border text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 ${
                  erros.razao_social ? 'border-red-500 bg-red-50' : 'border-slate-300'
                }`}
              />
              {erros.razao_social && (
                <p className="mt-1 text-xs text-red-500">{erros.razao_social}</p>
              )}
            </div>
...
// ---------------------------------------------------------------------------
// Componente principal: Listagem de Empresas
// ---------------------------------------------------------------------------

export default function ListagemEmpresas() {
  const [empresas, setEmpresas] = useState<EmpresaListada[]>([]);
  const [carregando, setCarregando] = useState(true);
  const [empresaEditando, setEmpresaEditando] = useState<EmpresaListada | null>(null);
  const [desativandoId, setDesativandoId] = useState<number | null>(null);
  const { toasts, adicionarToast, removerToast } = useToast();

  // Busca empresas ao montar o componente
  const carregarEmpresas = async () => {
    setCarregando(true);
    try {
      const resposta = await empresaService.listar();
      setEmpresas(resposta.data.dados ?? []);
    } catch {
      adicionarToast('Erro ao carregar a lista de empresas.', 'erro');
    } finally {
      setCarregando(false);
    }
  };
  ...
  // Desativa logicamente uma empresa (soft delete via DELETE)
  const handleDesativar = async (empresa: EmpresaListada) => {
    const confirmou = window.confirm(
      `Desativar "${empresa.razao_social}"?\nO registro será mantido no banco com status inativo.`
    );
    if (!confirmou) return;

    setDesativandoId(empresa.id_empresa);
    try {
      await empresaService.desativar(empresa.id_empresa);
      adicionarToast(`Empresa "${empresa.razao_social}" desativada com sucesso!`, 'sucesso');
      // Remove da lista local sem precisar rebuscar do servidor
      setEmpresas((prev) => prev.filter((e) => e.id_empresa !== empresa.id_empresa));
    } catch (err: unknown) {
      const mensagem = axios.isAxiosError(err)
        ? (err.response?.data?.mensagem ?? 'Erro ao desativar empresa.')
        : 'Erro inesperado.';
      adicionarToast(mensagem, 'erro');
    } finally {
  ...
  const handleSalvoEdicao = () => {
    adicionarToast('Empresa atualizada com sucesso!', 'sucesso');
    setEmpresaEditando(null);
    void carregarEmpresas(); // Recarrega lista para refletir alterações
  };

  // Formata CNPJ adicionando máscara visual: 00.000.000/0000-00
    ...
    <>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mb-8 flex items-start justify-between flex-wrap gap-4">
            <div>
              <div className="inline-flex items-center gap-2 bg-indigo-100 text-indigo-700 text-xs font-semibold px-3 py-1 rounded-full mb-3">
                <span>🏢</span> Sistema de Moeda Estudantil
              </div>
              <h1 className="text-3xl font-bold text-slate-900 tracking-tight">
                Empresas Parceiras
              </h1>
              <p className="mt-1 text-slate-500 text-sm">
                Lista de empresas ativas. Use os botões para editar dados ou desativar um cadastro.
              </p>
            </div>
            <button
          ...
          {carregando && empresas.length === 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-slate-400">
              ...
              <p className="text-base font-medium text-slate-600">Nenhuma empresa cadastrada ainda.</p>
              <p className="text-sm mt-1">Cadastre a primeira empresa pela tela de Cadastro.</p>
            </div>
          ) : (
            /* Tabela */
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200">
                      ...
                      <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Contato</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {empresas.map((empresa) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <BadgeStatus ativo={empresa.usuario.ativo} />
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <button
                              ...
                              className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-indigo-50 text-indigo-700 text-xs font-semibold hover:bg-indigo-100 transition-colors"
                            ...
                            {/* Botão Desativar */}
                            <button
                              onClick={() => void handleDesativar(empresa)}
                              disabled={desativandoId === empresa.id_empresa}
                              ...
                              title="Desativar empresa (exclusão lógica)"
                            >
                              {desativandoId === empresa.id_empresa ? (
                                <Spinner tamanho="sm" />
                              ) : (
                                '🚫'
                              )}
      ...
      {empresaEditando && (
        <ModalEdicao
          ...
          onFechar={() => setEmpresaEditando(null)}
